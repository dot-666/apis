...
// ── Audio ─────────────────────────────────────────────────────────────────────
// Uses the same audio conversion flow as the YTMP3 frontend.
// Returns { downloadUrl, title, duration, thumbnail }
async function getAudioUrl(videoId, quality = '128') {
    const meta = await getInfo(videoId);
    const dlRes = await axios.post(
        `https://${meta._cdn}/download`,
        { downloadType: 'audio', quality, key: meta.key },
        { headers: HEADERS, timeout: 12000 }
    );
    const downloadUrl = dlRes.data?.data?.downloadUrl;

    if (!downloadUrl) throw new Error('savetube did not return an audio downloadUrl');
...
async function getVideoUrl(videoId) {
    ...
    for (const q of queue) {
        const dlRes = await axios.post(
            `https://${meta._cdn}/download`,
            { downloadType: 'video', quality: q, key: meta.key },
            { headers: HEADERS, timeout: 12000 }
        );
